...
def run_tests():
    ...
    # Use an isolated temporary database for test suite so live database is never polluted
    with tempfile.NamedTemporaryFile(suffix="_fraudguard_test.db", delete=False) as tf:
        test_db_path = tf.name

    os.environ["FRAUDGUARD_DB_PATH"] = test_db_path
    ...
    try:
        ...
        if "FRAUDGUARD_DB_PATH" in os.environ:
            del os.environ["FRAUDGUARD_DB_PATH"]
